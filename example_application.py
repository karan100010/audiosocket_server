...
class AudioStreamer():
  def __init__(self,socket,call):
    ...
    try:
      ...
      self.logger.info(e)
    req=self.conn["test"]["calls"].insert_one({"status":"active","addr":self.audiosocket.addr+":"+str(self.audiosocket.port),"conn":0,"time_updates":datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")})
