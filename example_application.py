...
class AudioStreamer():
  def __init__(self,socket,call):
    ...
    try:
      ...
      self.logger.info(e)
    try:
      self.conn["test"]["connections"].insert_one({"status":"active","addr":"172.16.1.209"+":"+str(self.audiosocket.port),"conn":0,"time_updates":datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")})
    except Exception as e:
      self.logger.info(e) 
  ...
  def start_audio_playback(self,mapping):
    ...
    if self.call.connected:
       ...
       self.conn["test"]["connections"].update_one({ "addr":self.audiosocket.addr+":"+str(self.audiosocket.port)},{"$set":{"conn":self.num_connected}})
    while self.call.connected:
        #
...
def handel_call():
  ...
  audiosocket=Audiosocket(("localhost",8010))
  while True:
